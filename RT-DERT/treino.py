...
model.train(
    ...
    # ❄️ Congelar backbone apenas no início (DETR converte melhor assim)
    freeze=0,

    name=RUN_NAME,
    optimizer="AdamW",

    # 📉 Aprendizado mais estável
    lr0=0.0002,       # LR inicial menor (DETR gosta de LR baixo)
    lrf=0.01,
    weight_decay=0.0005,
    patience=20,

    # 🔁 Aumentações suaves (DETR NÃO gosta de distorção agressiva)
    flipud=0.1,
    fliplr=0.5,
    ...
    scale=0.2,        # antes 0.5 (muito forte)
    translate=0.1,    # antes 0.5
    shear=0.0,        # DESATIVE — DETR detesta shear
    mosaic=0.2,       # antes 1.0 (extremamente agressivo)
    mixup=0.1,        # antes 0.5
    copy_paste=0.0,   # DETR raramente se beneficia disso
